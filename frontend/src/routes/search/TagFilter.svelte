<script lang="ts">
	...
	const getTag = debounce(async (query: string) => {
		await timeout(1000);
		return await search.index('tags').search(query, {
			sort: ['frequency:desc']
		});
	}, 1250);
...
<div class="flex flex-col w-full px-2 pb-1">
	...
	>
		<label for="tag-filer" class="bg-zinc-700 h-full w-full pl-2">Tags:</label>
	</div>
	{#if value.length}
		<div class="flex flex-col w-full gap-y-2 mb-2 border-zinc-800 border">
			{#each value as el}
